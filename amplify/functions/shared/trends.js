...
export function dateIdInToronto(offsetDays = 0) {
...
function deterministicWeightedSampleTop(items, sampleSize, seedKey) {
    const rng = mulberry32(hashString(seedKey));
    const pool = [...items];
    const out = [];
...
function isLikelyLowSignal(term, score) {
    ...
    if (GENERIC_LOW_SIGNAL.has(normalized) && score < 90)
        return true;
    ...
    if (hasDigit || isAcronym)
        return score < 30;
    // Low-signal generic single words are filtered more aggressively.
    if (isSingleWord && /^[a-z]+$/.test(normalized) && score < 55)
        return true;
...
function parseDailyRssTitles(xmlText) {
    const out = [];
...
async function fetchRssDailyCandidates() {
    const merged = [];
...
async function fetchLegacyDailyCandidates() {
    const urls = [
...
async function rankTermsByConfiguredProvider(terms, seedKey) {
...
async function fetchRelatedTerms(seed) {
    let widgets = [];
    try {
...
export async function generateTrendPuzzle(options) {
    ...
    const pickedTerms = deterministicWeightedSampleTop(topWindow, TREND_ITEMS_PER_PUZZLE, `${options.puzzleId}:pick`);
    if (pickedTerms.length < TREND_ITEMS_PER_PUZZLE) {
        throw new Error("Unable to build 5 unique trend items.");
    }
    const relevancePool = pickedTerms.map((term, idx) => ({
        term,
        score: Math.max(1, 100 - idx),
        isTopSignal: true,
    }));
    const googleScores = await rankTermsByConfiguredProvider(relevancePool.map((x) => x.term), options.puzzleId);
    const scored = relevancePool.map((entry) => {
        const googleScore = googleScores.get(entry.term);
        return {
            ...entry,
            score: Number.isFinite(googleScore) ? googleScore : entry.score,
        };
    });
